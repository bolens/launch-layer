...
`lib/runtime/` includes `tuning.sh` (env and GPU selection), `chain.sh` (wrappers), `inject.sh` (cache/NOTICE/track plus exclusive proxy-provider claims), and `extras.sh` (Special K, ReShade, OptiScaler, Conty, VR, winetricks, …). Injector implementations share the provider and tracked backup/restore boundary instead of independently overwriting game files. The launch PID and AppID are persisted together so the watchdog can restore tracked files after a force-quit. Upstream license policy: [third-party.md](third-party.md).
