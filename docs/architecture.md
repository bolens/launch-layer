...
`lib/runtime/` includes `tuning.sh` (env and GPU selection), `chain.sh` (wrappers), `inject.sh` (cache/NOTICE/track plus exclusive proxy-provider claims), and `extras.sh` (Special K, ReShade, OptiScaler, Conty, VR, winetricks, …). Injector implementations share the provider and transactional tracked-operation boundary instead of independently overwriting game files; multi-file providers roll back the whole operation when one file fails. Fetches and archive extraction are bounded by byte and member limits and reject links before extraction. The launch PID and AppID are persisted before host tuning or injection starts so the watchdog can restore tracked files after a force-quit. Upstream license policy: [third-party.md](third-party.md).
...
Report ranking uses a deterministic score, recency tie-break, and numeric Proton version ordering. Automatic changes use reports from the last year, require corroboration, validate keys and values against a strict policy, keep complete post-`%command%` argument groups, and skip Proton overrides for native games unless the effective config sets `FORCE_PROTON=1`. Reports without `%command%` cannot supply game arguments. Older reports remain available as read-only context.
