...
`--suggest-config --apply` only automates settings supported by a strict key and value allowlist. Recommendations must come from at least two reports no older than one year; compatibility fallbacks, complete game-argument groups, and custom Proton overrides require at least 35% of the matching report weight. Native games receive no Proton override unless their effective config declares `FORCE_PROTON=1`, and standard Valve Proton remains on Steam's default selection instead of receiving a permanent version pin. Game arguments are considered only after an explicit `%command%` marker. Preview may still show older comments, but they cannot change the config.
