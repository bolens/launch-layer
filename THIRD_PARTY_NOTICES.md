...
## Installed tools and cached artifacts

[The tool inventory](docs/third-party.md) records upstream projects and purchase
requirements. Invoking a separately installed program does not transfer its
license to LaunchLayer. Cached tools and injected libraries retain their own
terms. The cache's URL/version/SPDX summary is not a substitute for the full
license, copyright notices, or corresponding source required for redistribution.
Do not redistribute a cache as though it were covered by LaunchLayer's MIT grant.
